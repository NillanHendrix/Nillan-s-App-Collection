...
- Existing apps: `notes` (order 10), `links` (15), `counter` (20, no DB table), `monitor` (25), `wow` (35) (WoW companion: characters, Mythic+, weekly checklist, ilvl/rating history, tracked achievements via `api/wow.ts`; nested routes `apps/wow/:charId`; tables `wow_characters`, `wow_snapshots`, `wow_goals`, `wow_achievements`, `0006_wow.sql`), and `about` (40) (a wiki-style Markdown editor with `[[wikilinks]]`, autosave, and a small self-written renderer in `markdown.ts` that escapes HTML first; table `about_pages`, `0004_about.sql`). Migrations are `0001_notes` through `0006_wow` (`0005` is `todo`), and the next number is `0007`.
